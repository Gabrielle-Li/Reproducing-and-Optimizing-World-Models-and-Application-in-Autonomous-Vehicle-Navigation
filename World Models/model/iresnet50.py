...
class IBasicBlock(nn.Module):  #基本块
    expansion = 1
...
class IResNet(nn.Module):                                          #网络框架
    ...
    def __init__(self, block, layers, num_classes, zero_init_residual=False,
        ...
        self.bn2 = nn.BatchNorm2d(
            64 * block.expansion, eps=2e-05, momentum=0.9)
...
def iresnet18(pretrained=False, progress=True, **kwargs):  #构建网络
    model = IResNet(block=IBasicBlock, layers=[2, 2, 2, 2], **kwargs)
    return model


def iresnet50(pretrained=False, progress=True, **kwargs):  #构建网络
    model = IResNet(block=IBasicBlock, layers=[3, 4, 14, 3], **kwargs)
    return model
