...
parser.add_argument('--map_list', default='maplist.txt', type=str,
                    help='全局训练集列表')
parser.add_argument('--map_val', default='mapval.txt', type=str,
                    help='全局验证集列表')
...
def train_map(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, (img) in enumerate(train_bar):
            img = img.cuda(args.devide)
            # 模型计算
            features, fake_img = model(img)

            loss = criterion(features, fake_img)
            train_loss_sum += loss
            count += 1
        ...
        with torch.no_grad():
            ...
            for val_data in val_bar:
                val_images = val_data
                val_features, val_fake = model(val_images.cuda(args.device))
                val_loss = criterion(val_features, val_fake)
                j += 1
